...
- A conda environment needs to be created using the "vs2_env.txt" file  
		` conda create -n vs --file /path/to/vs2_env.txt`  
		` conda activate vs`
...
#### Run VirusSeeker:
```
perl runVS.pl [output path]/[run name] 1 [slurm CPUS] 0 0 D [s/l/h (read type)] [m/i (metagenomic vs isolate)]		
```
- See runVS.pl script for argument options	
