...
- A conda environment needs to be created using the "vs.yml" file  
		` conda env create --file /path/to/vs.yml`  
		` conda activate vs`
...
- Due to dependency issues, another environment for "dragonflye" will also need to be created  
  		` conda env create --file /path/to/dragonflye.yml`
...
```
#### Run VirusSeeker:
- sample_dir, ref_genome_choice, num_CPU, slurm_mem, assembly_type, assembly_mode, use_checkpoint, step_number, VStrack
```
perl runVS.pl [directory containing sequenced reads for 1 sample] ["/path/to/host/reference.fasta"] [slurm CPUS] [read type] [sequencing type] 0 0 D  
```
