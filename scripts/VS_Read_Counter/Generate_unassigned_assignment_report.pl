...
####################################
# output files 
my $out3 = $dir."/".$sample_name.".unassignedReadsAssignmentReport";
open (OUT3, ">$out3") or die "can not open file $out3!\n";
my $out4 = $dir."/".$sample_name.".unassignedReads_all.fa";
...
# all the unassigned viral read ID
my %unassigned_reads_MegaBlast_NT = ();
my %unassigned_reads_blastn = ();
my %unassigned_reads_blastx = ();

# unassigned reads viral lineage => number of reads assigned to this lineage in the sample
my %unassigned_reads_lineage_to_num_reads = ();
my %unassigned_reads_blast_info = ();    # readID => information about this read
my %unassigned_reads_in_lineage_MegaBlast_NT = ();    # lineage => [read ID]
my %unassigned_reads_in_lineage_blastn = ();    # lineage => [read ID]
my %unassigned_reads_in_lineage_blastx = ();    # lineage => [read ID]
...
foreach my $blast_file (readdir DIR) {
	if ($blast_file =~ /megablast\.parsed$/) {
		...
		&collect_information($parsed, \%Assignment_MegaBlast_NT, \%viral_reads_MegaBlast_NT, \%viral_reads_in_lineage_MegaBlast_NT, \%viral_lineage_to_num_reads, \%unassigned_reads_MegaBlast_NT, \%unassigned_reads_in_lineage_MegaBlast_NT, \%unassigned_reads_lineage_to_num_reads, \@unassigned_reads, \%viral_reads_blast_info, \%unassigned_reads_blast_info, \@phage_reads );
	}
}
closedir DIR;
...
foreach my $blast_file (readdir BNDIR) {
	if ($blast_file =~ /blastn\.parsed$/) {
		...
		&collect_information($parsed, \%Assignment_blastn, \%viral_reads_blastn, \%viral_reads_in_lineage_blastn, \%viral_lineage_to_num_reads, \%unassigned_reads_blastn, \%unassigned_reads_in_lineage_blastn, \%unassigned_reads_lineage_to_num_reads, \@unassigned_reads, \%viral_reads_blast_info, \%unassigned_reads_blast_info );
	}
}
closedir BNDIR;
...
foreach my $blast_file (readdir BXDIR) {
	if ($blast_file =~ /blastx\.parsed$/) {
		...
		&collect_information($parsed, \%Assignment_blastx, \%viral_reads_blastx, \%viral_reads_in_lineage_blastx, \%viral_lineage_to_num_reads, \%unassigned_reads_blastx, \%unassigned_reads_in_lineage_blastx, \%unassigned_reads_lineage_to_num_reads, \@unassigned_reads, \%viral_reads_blast_info, \%unassigned_reads_blast_info );
	}
}
closedir BXDIR;
...
# generate unassigned reads report
print OUT3 $c1;
foreach my $lineage (sort {$unassigned_reads_lineage_to_num_reads{$a} <=> $unassigned_reads_lineage_to_num_reads{$b}} keys %unassigned_reads_lineage_to_num_reads) {
	print OUT3 $lineage, "\ttotal number of reads: ", $unassigned_reads_lineage_to_num_reads{$lineage}, "\n\n";
	print OUT3 "QueryName\tQuerylength\t         HitName       \tHitLen\t                             HitDesc                       \tAlnLen\t%ID\tHitStart\tHitEnd\te\n";

	if (defined $unassigned_reads_in_lineage_MegaBlast_NT{$lineage}) {
		if (scalar @{$unassigned_reads_in_lineage_MegaBlast_NT{$lineage}}) {
			print OUT3 "reads from MegaBlast NT:\n";
			foreach my $read (sort {$a cmp $b} @{$unassigned_reads_in_lineage_MegaBlast_NT{$lineage}}) {
				print OUT3 $unassigned_reads_blast_info{$read}, "\n";
			}
		}
	}


	if (defined $unassigned_reads_in_lineage_blastn{$lineage}) {
		if (scalar @{$unassigned_reads_in_lineage_blastn{$lineage}}) {
			print OUT3 "reads from blastn:\n";
			foreach my $read (sort {$a cmp $b} @{$unassigned_reads_in_lineage_blastn{$lineage}}) {
				print OUT3 $unassigned_reads_blast_info{$read}, "\n";
			}
		}
		print OUT3 "\n";
	...
	if (defined $unassigned_reads_in_lineage_blastx{$lineage}) {
		if (scalar @{$unassigned_reads_in_lineage_blastx{$lineage}}) {
			print OUT3 "reads from blastx:\n";
			foreach my $read (sort {$a cmp $b} @{$unassigned_reads_in_lineage_blastx{$lineage}}) {
				print OUT3 $unassigned_reads_blast_info{$read}, "\n";
			}
		}
		print OUT3 "\n";
...
###############################################################################
# get all the unassigned reads and put the sequence into output file:
foreach my $lineage (keys %unassigned_reads_lineage_to_num_reads) {
	foreach my $read (@{$unassigned_reads_in_lineage_MegaBlast_NT{$lineage}}) {
		print OUT4 ">$read $seq_desc{$read}\n";
		print OUT4 $seq{$read}, "\n";
	}

	foreach my $read (@{$unassigned_reads_in_lineage_blastn{$lineage}}) {
		print OUT4 ">$read $seq_desc{$read}\n";
		print OUT4 $seq{$read}, "\n";
	}
	foreach my $read (@{$unassigned_reads_in_lineage_blastx{$lineage}}) {
		print OUT4 ">$read $seq_desc{$read}\n";
		print OUT4 $seq{$read}, "\n";
	}
#	foreach my $read (@{$unassigned_reads_in_lineage_blastx_VIRUSDB{$lineage}}) {
#		print OUT4 ">$read $seq_desc{$read}\n";
#		print OUT4 $seq{$read}, "\n";
#	}
}
...
sub collect_information {
	################################## modified 04/2014
	my ($blast_parsed_file, $category_hash_ref, $viral_reads_hash_ref, $viral_reads_in_lineage_hash_ref, $viral_lineage_to_num_reads_hash_ref,  $unassigned_reads_hash_ref, $unassigned_reads_in_lineage_hash_ref, $unassigned_reads_lineage_to_num_reads_hash_ref, $unassigned_reads_arr_ref, $viral_reads_blast_info_ref, $unassigned_reads_blast_info_ref ) = @_;
...
#		print "readID = \\$read_ID\\, length = \\$length\\, category = \\$category\\, lineage = \\$lineage\\, desc = \\$desc\\\n";
		...
		################################## modified 2014.04
			...
			$unassigned_reads_hash_ref->{$read_ID} = 1;
			$unassigned_reads_blast_info_ref->{$read_ID} = $desc;
			if (!(defined $unassigned_reads_in_lineage_hash_ref->{$lineage})) {
				$unassigned_reads_in_lineage_hash_ref->{$lineage} = [$read_ID];
			}
			else {
				push @{$unassigned_reads_in_lineage_hash_ref->{$lineage}}, $read_ID;
			}
		
			if (defined $unassigned_reads_lineage_to_num_reads_hash_ref->{$lineage}) {
				$unassigned_reads_lineage_to_num_reads_hash_ref->{$lineage}++;
			}
			else {
				$unassigned_reads_lineage_to_num_reads_hash_ref->{$lineage} = 1;
			}
