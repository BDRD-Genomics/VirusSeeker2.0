...
my $metaSPAdes = "spades.py --only-assembler --meta";
my $SPAdes = "spades.py --only-assembler --isolate";
...
sub metaSPAdes_assembly{
	...
	# If assembly directory already exists, the assembly failed previously. Need to remove old assembly directory.
        ...
        print STCH "            fi\n";
        print STCH "            $metaSPAdes --tmp-dir \${OUTDIR} --memory $SPAdes_mem -t \$SLURM_CPUS_PER_TASK -1 \${IN1} -2 \${IN2} -o \${OUTDIR} \n";
        print STCH "            OUT=\$?\n";
        print STCH '            if [ ${OUT} -ne 0  ]',"\n"; # did not finish successfully
        print STCH "            then\n";
...
sub SPAdes_assembly{
        ...
        print STCH "            $SPAdes --tmp-dir \${OUTDIR} -t \$SLURM_CPUS_PER_TASK --memory $SPAdes_mem -1 \${IN1} -2 \${IN2} -o \${OUTDIR} \n";
        print STCH "            OUT=\$?\n";
        print STCH '            if [ ${OUT} -ne 0  ]',"\n"; # did not finish successfully
        print STCH "            then\n";
...
sub run_mmseqs{
	...
	print STCH "			cp ${sample_dir}/${sample_name}.allSeqs.mmseqs_rep_seq.fasta ${sample_dir}/${sample_name}.goodSeq.fa\n";
	print STCH "		fi\n";
	print STCH "	fi\n";
	print STCH "fi\n";
	close STCH;
