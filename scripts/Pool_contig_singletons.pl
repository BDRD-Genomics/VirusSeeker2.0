...
<input singleton fasta file> = sequence file with full path

output to standard output.

';
die $usage unless scalar @ARGV == 2;
my ($F_contig, $F_seq) = @ARGV;
...
# For the singleton sequence file just read and write 
open (IN, $F_seq) or die "Can't Open FASTA file: $F_seq!\n";
while (my $line = <IN>){
	print "$line";
}
close IN;
