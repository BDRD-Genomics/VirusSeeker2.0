...
sub check_split_output {
	...
	my $in_file = $dir."/".$sample_name."_SingletonOutlier.fastq";
	my $total_lines = `wc -l < $in_file`;
	$tot_in_seq = $total_lines / 4;

	my $Assembly_dir = $dir."/".$sample_name."_Assembly_Second";
	opendir(DH, $Assembly_dir) or return 10;
	foreach my $file (readdir DH) {
		if ($file =~ /\.fastq$/) { 
			my $in = $Assembly_dir."/".$file;
