...
sub check_split_output {
	...
	my $BLAST_dir = $dir."/".$sample_name."_MMseqs_VIRUSDB";
	...
	foreach my $file (readdir DH) {
		...
		if ($file =~ /\.mmseqsv\.parsed$/ ) {	
			my $file_name = $file;
			#$file_name =~ s/fasta$/[0-9]+\.mmseqs\.parsed/;
		...
		if ($file =~ /\.mmseqsv\.out\.\d?$/ ) {
			# get blast output file not finished
			my $file_name = $file;
			$file_name =~ s/fasta$/mmseqsv\.out\.[0-9]+/;
