...
sub check_split_output {
	...
	my $in_file = $dir."/".$sample_name."_pair_corrected_PE.fastq";
	my $total_lines = `wc -l < $in_file`;
	$tot_in_seq = $total_lines / 4;

	my $Assembly_dir_PE = $dir."/".$sample_name."_Assembly_PE";
	opendir(DH, $Assembly_dir_PE) or return 10;
	foreach my $file (readdir DH) {
		if ($file =~ /\.fastq$/) { 
			my $in = $Assembly_dir_PE."/".$file;
	...
	if($tot_in_seq==$tot_out_seq) { return 0; } # use 0 to be consistent with Linux convention 
		...
		opendir(DH, $Assembly_dir_PE) or return 10;
		foreach my $file (readdir DH) {	
			my $faFile = $Assembly_dir_PE."/".$file;
   #         print "$faFile\n"; 
			unlink $faFile; 
		}
	  	close DH;  
